...
CDLXSIDEGAP3METHODS_COLUMN = "cdlxsidegap3methods"


def compute_cdlxsidegap3methods(frame: pl.DataFrame) -> pl.DataFrame:
    ordered = (
    ...
    return ordered.select(
        "trade_date",
        "ts_code",
        pl.when(bullish).then(1).when(bearish).then(-1).otherwise(0).alias(CDLXSIDEGAP3METHODS_COLUMN),
    )
