...
def compute_average_true_range_14(frame: pl.DataFrame) -> pl.DataFrame:
    r"""
    真实振幅 14 日移动平均，Tushare 对应 ATR14，参数 N=14。
    ...
    return tr_frame.select(
        "trade_date",
        "ts_code",
        pl.col("tr_14_base").rolling_mean(window_size=ATR_WINDOW).over("ts_code").alias(ATR_COLUMN),
    )
