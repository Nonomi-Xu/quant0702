...
CDLINVERTEDHAMMER_COLUMN = "cdlinvertedhammer"


def compute_cdlinvertedhammer(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    return ordered.select(
        "trade_date",
        "ts_code",
        pl.when(signal).then(1).otherwise(0).alias(CDLINVERTEDHAMMER_COLUMN),
    )
