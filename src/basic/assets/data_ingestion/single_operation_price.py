...
class MyAssetConfig(dg.Config):
    # 添加你需要的参数，可以设置默认值
    target_symbol: str = None  # 默认股票代码

@dg.asset(
    group_name="data_ingestion_single_operation",
    description="单次更新指定A股日线数据",
)
def Data_Ingestion_Single_Operation_Price(context: dg.AssetExecutionContext, config: MyAssetConfig) -> dg.MaterializeResult:
    """
    单次更新指定A股日线数据
    """
    ...
    last_date = conn.execute("""
                    SELECT MAX(date) AS date FROM daily_prices WHERE symbol = ?
                """, [symbol]).fetchdf()
    last_date = set(pd.to_datetime(last_date['date']).dt.strftime('%Y-%m-%d'))
    ...
    try:
        ...
        # 转换为pandas DataFrame
        pl_pd = pl_df.to_pandas()
        
        # 添加更新时间
        pl_pd['updated_at'] = datetime.now()
        
        # 注册临时表并插入
        conn.register("temp_daily_batch", pl_pd)
