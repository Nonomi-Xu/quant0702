...
CDLHANGINGMAN_COLUMN = "cdlhangingman"


def compute_cdlhangingman(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    return ordered.select(
        "trade_date",
        "ts_code",
        pl.when(signal).then(-1).otherwise(0).alias(CDLHANGINGMAN_COLUMN),
    )
