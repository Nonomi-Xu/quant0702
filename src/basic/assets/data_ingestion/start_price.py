...
from .start_org import Add_Delisting_Stocks


@dg.asset(
    group_name="data_ingestion_first_time_price",
    description="创建A股日线数据（使用a_stock_all表）",
...
def Start_Basic_Prices(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    ...
    db.reset_tables(table_name = 'basic_daily_prices', drop_data=True)
    ...
    start_date_ak = datetime.strptime(start_date, '%Y-%m-%d').strftime('%Y%m%d')
    end_date_ak = datetime.strptime(end_date, '%Y-%m-%d').strftime('%Y%m%d')
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        for symbol in batch_symbols:
            try:
                ...
                existing_dates = conn.execute("""
                    SELECT date FROM basic_daily_prices
        ...
        if batch_dfs:
            try:
                ...
                conn.execute("""
                    INSERT INTO basic_daily_prices (
