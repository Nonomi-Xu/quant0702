...
CDLIDENTICAL3CROWS_COLUMN = "cdlidentical3crows"


def compute_cdlidentical3crows(frame: pl.DataFrame) -> pl.DataFrame:
    ordered = (
    ...
    signal = (
        ...
        & ((pl.col("open_2") - pl.col("close_1")).abs() <= (pl.col("open_1") - pl.col("close_1")).abs() * 0.2)
        & ((pl.col("open_hfq") - pl.col("close_2")).abs() <= (pl.col("open_2") - pl.col("close_2")).abs() * 0.2)
    ...
    return ordered.select(
        "trade_date",
        "ts_code",
        pl.when(signal).then(-1).otherwise(0).alias(CDLIDENTICAL3CROWS_COLUMN),
    )
