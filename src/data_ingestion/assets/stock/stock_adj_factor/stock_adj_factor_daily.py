...
FILE_PATH_FRONT = "data/stock/stock_adj_factor/stock_adj_factor/"
FILE_NAME = "stock_adj_factor"

@dg.asset(
    group_name="data_ingestion_daily",
    description="每日获取A股复权因子 增量写入",
    deps=[Trade_Cal_Daily]
)
def Stock_Adj_Factor_Daily(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """
    每日获取A股复权因子 增量写入
    """
    ...
    for idx, trade_date in enumerate(date_list, start=1):
        ...
        df = tushare_api.adj_factor(ts_code='', trade_date=trade_date)
...
def load_adj_factor(
    ...
    for source_year in year_list:
        ...
        file_path_daily = FILE_PATH_FRONT + FILE_NAME + f"_{source_year}.parquet"
        try:
