...
CDLEVENINGSTAR_COLUMN = "cdleveningstar"


def compute_cdleveningstar(frame: pl.DataFrame) -> pl.DataFrame:
    ordered = (
    ...
    small_body_2 = (pl.col("close_2") - pl.col("open_2")).abs() <= (pl.col("close_1") - pl.col("open_1")).abs() * 0.4
    signal = (
    ...
    return ordered.select(
        "trade_date",
        "ts_code",
        pl.when(signal).then(-1).otherwise(0).alias(CDLEVENINGSTAR_COLUMN),
    )
