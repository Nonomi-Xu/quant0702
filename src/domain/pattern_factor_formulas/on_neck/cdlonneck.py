...
CDLONNECK_COLUMN = "cdlonneck"


def compute_cdlonneck(frame: pl.DataFrame) -> pl.DataFrame:
    ordered = (
    ...
    signal = (
        ...
        & ((pl.col("close_hfq") - pl.col("open_1")).abs() <= pl.col("open_1").abs() * 0.01)
    )
    return ordered.select(
        "trade_date",
        "ts_code",
        pl.when(signal).then(-1).otherwise(0).alias(CDLONNECK_COLUMN),
    )
